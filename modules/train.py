...
class RETrainer(BaseTrainer):
    ...
    def train(self):
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    ...
                    if self.step % self.refresh_step == 0:
                        ...
                        avg_loss = 0
                total_loss=total_loss/len(self.train_data)
        ...
        with open('train_loss.txt','w') as f:
            for this_loss in self.train_loss_list:
                f.write(str(this_loss)+'\n')
        with open('val_loss.txt','w') as f:
            for this_loss in self.val_loss_list:
                f.write(str(this_loss)+'\n')
        with open('val_f1.txt','w') as f:
            for this_f1 in self.eval_f1:
                f.write(str(this_f1)+'\n')
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                self.val_loss_list.append(total_loss/len(self.test_data))
                self.eval_f1.append(micro_f1)
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.test_data:
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)  # to cpu/cuda device  
                    (loss, logits), labels, seqs, images,relation, head_pos, tail_pos = self._step(batch, mode="dev")    # logits: batch, 3
                    total_loss += loss.detach().cpu().item()
                    ...
                    for i in range(len(true_tmp)):
                        ...
                        check.append(
                            [seqs[i], images[i], head_pos['name'][i], tail_pos['name'][i], re_dic[true_tmp[i]],
                             re_dic[pred_tmp[i]]])
                df_ot = pd.DataFrame(check)
                df_ot.columns = ['seq', 'img', 'head_pos', 'tail_pos', 'true_label', 'pre_label']
                df_ot.to_csv('./check.csv', index=False)
                print('输出check成功！')
                # evaluate done
                pbar.close()
                sk_result = sk_classification_report(y_true=true_labels, y_pred=pred_labels, labels=list(self.re_dict.values())[1:], target_names=list(self.re_dict.keys())[1:], digits=4)
    ...
    def _step(self, batch, mode="train"):
        if mode != "predict":
            if self.args.use_prompt:
                ...
                input_ids, token_type_ids, attention_mask, labels, images, aux_imgs, seq, img, relation, seq_input_ids, seq_token_type_ids, seq_attention_mask,head_pos,tail_pos = batch
            else:
            ...
            return outputs, labels,seq, img, relation,head_pos,tail_pos
...
class NERTrainer(BaseTrainer):
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                self.val_loss_list.append(total_loss/step)
                self.eval_f1.append(f1_score)
    ...
    def test(self):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.test_data:
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)
                    attention_mask, labels, logits, loss,seqs,imgs = self._step(batch, mode="dev")
                    total_loss += loss.detach().cpu().item()
                    ...
                    for row, mask_line in enumerate(input_mask):
                        ...
                        y_pred.append(true_predict)
                        # ------------------------------------#
                        check.append([seqs[row], imgs[row], true_label, true_predict])
                    pbar.update()
                df_ot = pd.DataFrame(check)
                df_ot.columns = ['seq', 'img', 'true_label', 'pre_label']
                df_ot.to_csv('./check_ner_val_17.csv',encoding='utf-8', index=False)
                # ------------------------------------
                #     pbar.update()
                # evaluate done
                pbar.close()
    ...
    def _step(self, batch, mode="train"):
        if self.args.use_prompt:
            input_ids, token_type_ids, attention_mask, labels, images, aux_imgs,seq_input_ids, seq_token_type_ids, seq_attention_mask,seq, img = batch
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                                labels=labels, images=images, aux_imgs=aux_imgs, seq_input_ids=seq_input_ids,
                                seq_token_type_ids=seq_token_type_ids, seq_attention_mask=seq_attention_mask,
                                )#attention_weights=attention_weights

        else:
            images, aux_imgs = None, None
            input_ids, token_type_ids, attention_mask, labels = batch#,attention_weights
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                                labels=labels, images=images, aux_imgs=aux_imgs,
                                )#attention_weights=attention_weights
        logits, loss = output.logits, output.loss
        return attention_mask, labels, logits, loss,seq, img
