...
#E:\Tool\Anaconda\envs\RE_cuda\Lib\site-packages\transformers\models\clip
...
class CLIPEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([CLIPEncoderLayer(config) for _ in range(config.num_hidden_layers)])#从这里拿四层或者12层的信息
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                hidden_states = layer_outputs[0]#1,50,768
    ...
    def clip_FPN_encoder(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if idx in need_list:#4层
                #     all_layers_result.append(pooled_output)#4层
                all_layers_result.append(pooled_output)#12层
        return all_layers_result
...
class CLIPVisionTransformer(nn.Module):
    ...
    def CLIPVisionTransformer_FPN(
        ...
        encoder_FPN_outputs = self.encoder.clip_FPN_encoder(
            inputs_embeds=hidden_states,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
...
class CLIPVisionModel(CLIPPreTrainedModel):
    ...
    def get_FPN_vision_feature(
        ...
        return self.vision_model.CLIPVisionTransformer_FPN(
            pixel_values=pixel_values,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def get_image_features(
        ...
        pooled_output = vision_outputs[1]  # pooled_output bsz,768
        image_features = self.visual_projection(pooled_output) #bsz,512
